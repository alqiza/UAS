...
// Query ambil produk kategori Makanan Pokok, dengan filter pencarian jika ada
$query = "SELECT * FROM t_produk 
          WHERE id_kategori = 250021 
    ...
    <div class="div">
      <div class="overlap">
        <div class="overlap-group">
          <!-- Bagian atas dan navigasi tetap -->
        ...
        <div class="overlap-7">
          ...
          while ($row_best = mysqli_fetch_assoc($result_best)) {
              if ($i >= 6) break;
              $id_produk = $row_best['id_produk'];
              $nama = htmlspecialchars($row_best['nama_produk']);
              $harga = number_format($row_best['harga_produk'], 0, ',', '.');
              $gambar = htmlspecialchars($row_best['gambar_produk']);

              echo '<div class="' . $nama_classes[$i] . '"><a href="info_produk.php?id=' . $id_produk . '">' . $nama . '</a></div>';
              echo '<div class="' . $harga_classes[$i] . '">Rp. ' . $harga . '</div>';
              echo '<a href="info_produk.php?id=' . $id_produk . '">';
