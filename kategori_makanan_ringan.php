...
// Query produk kategori Makanan Ringan
$query = "SELECT * FROM t_produk WHERE id_kategori = 250022 AND status_produk = 'tersedia'";
if (!empty($search)) {
    $query .= " AND nama_produk LIKE '%" . mysqli_real_escape_string($con, $search) . "%'";
}
...
$query_best = "
  ...
  FROM t_produk 
  WHERE is_best_seller = 1 AND status_produk = 'tersedia' 
  LIMIT 5
";
      ...
      <div class="div">
        <div class="overlap">
          <div class="overlap-group">
            ...
            <!-- Dua produk statik awal (boleh dihapus jika tak diperlukan) -->
            ...
            <p class="toko-dadi-rejo">
              ...
              jajanan, bahan dapur, minuman, hingga bahan makanan pokok.
            </p>

            <!-- Daftar Produk Makanan Ringan -->
          ...
          <div class="overlap-7">
            ...
            <?php
            $nama_classes  = ['text-wrapper-33', 'text-wrapper-34', 'text-wrapper-35', 'text-wrapper-36', 'text-wrapper-37'];
            $harga_classes = ['text-wrapper-27', 'text-wrapper-28', 'text-wrapper-29', 'text-wrapper-30', 'text-wrapper-31'];
            $gambar_classes= ['gula-removebg-2', 'energen-removebg-2', 'potabee-removebg', 'tehpucuk-removebg', 'untitled-removebg'];

            $i = 0;
            while ($row_best = mysqli_fetch_assoc($result_best)) {
                if ($i >= 5) break;
                $id    = $row_best['id_produk'];
                $nama  = htmlspecialchars($row_best['nama_produk']);
                $harga = number_format($row_best['harga_produk'], 0, ',', '.');
                $img   = htmlspecialchars($row_best['gambar_produk']);
                ...
                echo '<img class="' . $gambar_classes[$i] . '" src="' . $img . '" alt="' . $nama . '"/>';
                echo '</a>';
                $i++;
            }
            ?>
